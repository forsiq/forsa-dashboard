...
import type { Column, Action } from '@core/components/Data/DataTable.types';
              ...
              <div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                ...
                {(visibleActions?.length ?? 0) > 3 && (
                  <DataTableRowActionsMenu
                    row={row}
                    rowId={id}
                    actions={visibleActions!.slice(3)}
                    isOpen={openActionId === id}
                    onOpenChange={(open) => onToggleAction(open ? id : '')}
                    dir={dir}
                    inline
                    triggerClassName="p-1.5 rounded-lg"
