...
import { isAcceptableImageFile } from '@core/lib/utils/imageFile';
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
    ...
    const handleFiles = useCallback(
      (selectedFiles: FileList | File[]) => {
        ...
        const remainingSlots = multiple
          ? Math.max(0, maxFiles - displayItems.length)
        ...
        if (remainingSlots === 0 && Array.from(selectedFiles).length > 0) {
          newErrors.push(
            t('upload.error_max_files') || `Maximum ${maxFiles} files allowed`,
          );
        }
      ...
      [maxSize, maxFiles, multiple, displayItems.length, onChange, t]
    );
