  ...
  if (auctionLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] space-y-4">
        ...
        <p className="text-zinc-muted font-black uppercase tracking-[0.3em] animate-pulse">{t('auction.detail.scanning_core') || 'Scanning Asset Core...'}</p>
      </div>
    );
  }
    ...
    return (
      <Card className="max-w-2xl mx-auto !p-12 text-center space-y-6 bg-obsidian-card border-danger/20">
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-black text-zinc-text uppercase tracking-tighter">{t('auction.detail.node_desynchronization') || 'Node Desynchronization'}</h2>
          <p className="text-zinc-muted font-bold uppercase tracking-tight text-sm">{t('auction.detail.node_not_found') || 'Requested asset identifier could not be located within the active matrix.'}</p>
        </div>
        <AmberButton onClick={() => navigate('/auctions')} variant="secondary" className="px-8 h-12 uppercase font-black">
          {t('auction.detail.return_infrastructure') || 'Return to Infrastructure'}
        </AmberButton>
      </Card>
    );
  }
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <Link to={`/auctions/edit/${auction.id}`}>
            <AmberButton className="h-12 bg-white text-black font-black uppercase tracking-widest rounded-xl px-8 hover:bg-zinc-200 active:scale-95 transition-all">
              {t('auction.detail.modify_node') || 'Modify Node'}
            </AmberButton>
          </Link>
        </div>
      </div>
          ...
          <Card className="!p-0 border-border bg-black overflow-hidden relative group aspect-video lg:aspect-auto h-[600px] shadow-2xl">
             ...
             <div className="absolute top-6 left-6 flex items-center gap-3">
               ...
               <div className="bg-black/60 backdrop-blur-xl border border-white/5 px-4 py-2 rounded-lg flex items-center gap-2">
                 ...
                 <span className="text-xs font-black text-white tracking-tighter">{t('auction.detail.live_monitoring') || 'LIVE MONITORING'}</span>
               </div>
             </div>
               ...
               <div className="flex items-end justify-between gap-6">
                 <div className="space-y-4 max-w-2xl">
                   <div className="flex items-center gap-3">
                     ...
                     <span className="text-xs font-black text-white uppercase tracking-[0.3em]">{t('auction.detail.physical_specification') || 'Physical Specification'}</span>
                   </div>
          ...
          <Card className="!p-8 bg-obsidian-card border-border shadow-xl space-y-8">
            <div className="flex items-center justify-between border-b border-white/[0.03] pb-6">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-sm font-black text-zinc-text uppercase tracking-[0.25em]">{t('auction.detail.high_frequency_telemetry') || 'High-Frequency Telemetry'}</h3>
              </div>
              <div className="flex items-center gap-6">
                <div className="text-right">
                  <span className="text-[10px] font-black text-zinc-muted uppercase tracking-widest">{t('auction.detail.total_cycles') || 'Total Cycles'}</span>
                  <p className="text-xl font-black text-zinc-text leading-none mt-1">{auction.totalBids || 0}</p>
                </div>
                <div className="w-px h-8 bg-white/5" />
                <div className="text-right">
                  <span className="text-[10px] font-black text-zinc-muted uppercase tracking-widest">{t('auction.detail.velocity_status') || 'Velocity Status'}</span>
                  <p className="text-xl font-black text-emerald-400 leading-none mt-1 uppercase tracking-tighter">{t('auction.detail.stable') || 'STABLE'}</p>
                </div>
              </div>
            </div>
            ...
            {bidsLoading ? (
              <div className="space-y-4 py-8 text-center">
                ...
                <p className="text-xs text-zinc-muted font-black uppercase tracking-widest">{t('auction.detail.parsing_history') || 'Parsing Transaction History...'}</p>
              </div>
            ) : bids?.length === 0 ? (
              <div className="py-16 text-center space-y-4 bg-white/[0.02] rounded-2xl border border-dashed border-white/5">
                ...
                <p className="text-sm font-bold text-zinc-muted uppercase tracking-widest">{t('auction.detail.no_interactions') || 'No Bid Interactions Detected'}</p>
              </div>
            ) : (
                ...
                {bids?.map((bid, index) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-black text-zinc-text uppercase tracking-tight">{bid.bidderName || 'Anonymous Participant'}</span>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-[9px] font-black text-brand uppercase tracking-widest mt-0.5">{t('auction.detail.interaction_sequence') || 'Interaction Sequence'} #{bids.length - index}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-8 relative">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-1">
                  <span className="text-[10px] font-black text-zinc-muted uppercase tracking-[0.2em] px-1">{t('auction.detail.current_premium') || 'Current Premium'}</span>
                ...
                <div className="text-right space-y-1">
                  <span className="text-[10px] font-black text-zinc-muted uppercase tracking-[0.2em]">{t('auction.detail.target_window') || 'Target Window'}</span>
                  <div className={cn(
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  <div className="flex items-center justify-between px-1">
                     <label className="text-[10px] font-black text-zinc-muted uppercase tracking-widest">{t('auction.detail.base_progression') || 'Base Progression Layer'}</label>
                     <span className="text-[9px] font-black text-zinc-muted uppercase tracking-tighter">{t('auction.detail.min_step') || 'Min Step'}: ${(auction.bidIncrement || 0).toLocaleString()}</span>
                  </div>
                ...
                >
                  ...
                  <span className="relative z-10">{t('auction.detail.authorize_bid') || 'Authorize Bid Sequence'}</span>
                </AmberButton>
              </div>
              ...
              {auction.buyNowPrice && (
                ...
                >
                  {t('auction.detail.instant_acquisition') || 'Instant Acquisition Protocol'} (${auction.buyNowPrice.toLocaleString()})
                </AmberButton>
              )}
              ...
              <div className="p-4 rounded-xl bg-obsidian-panel/40 border border-white/5 flex items-center gap-3">
                 ...
                 <span className="text-[10px] font-black text-zinc-muted uppercase tracking-[0.1em]">{t('auction.detail.encrypted_infrastructure') || 'End-to-End Encrypted Bidding Infrastructure'}</span>
              </div>
            </div>
          </Card>
          ...
          <Card className="!p-8 bg-obsidian-card border-border shadow-xl space-y-6">
            <div className="flex items-center gap-3 border-b border-white/[0.03] pb-6">
              <h3 className="text-sm font-black text-zinc-text uppercase tracking-[0.25em]">{t('auction.detail.infrastructure_logistics') || 'Infrastructure Logistics'}</h3>
            </div>
            ...
            <div className="space-y-5">
               <div className="flex items-center justify-between group">
                 <div className="flex items-center gap-3">
                   ...
                   <span className="text-[10px] font-black text-zinc-muted uppercase tracking-widest">{t('auction.detail.temporal_start') || 'Temporal Start'}</span>
                 </div>
              ...
              <div className="flex items-center justify-between group">
                 <div className="flex items-center gap-3">
                   ...
                   <span className="text-[10px] font-black text-zinc-muted uppercase tracking-widest">{t('auction.detail.node_termination') || 'Node Termination'}</span>
                 </div>
              ...
              <div className="flex items-center justify-between group">
                 <div className="flex items-center gap-3">
                   ...
                   <span className="text-[10px] font-black text-zinc-muted uppercase tracking-widest">{t('auction.detail.progression_delta') || 'Progression Delta'}</span>
                 </div>
              ...
              <div className="flex items-center justify-between group">
                 <div className="flex items-center gap-3">
                   ...
                   <span className="text-[10px] font-black text-zinc-muted uppercase tracking-widest">{t('auction.detail.asset_custodian') || 'Asset Custodian'}</span>
                 </div>
                 <span className="text-xs font-black text-zinc-text uppercase tracking-tight">{auction.winnerName || 'Strategic Division 11'}</span>
              </div>
            </div>
            ...
            <div className="pt-4">
              <AmberButton variant="secondary" className="w-full gap-2 font-black uppercase tracking-widest text-[10px] h-10 bg-obsidian-panel border-border active:scale-95 transition-all">
                 {t('auction.detail.download_manifest') || 'Download Asset Manifest'}
              </AmberButton>
            </div>
          </Card>
        </div>
