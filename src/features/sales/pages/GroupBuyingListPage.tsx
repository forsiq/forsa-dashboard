        ...
        <div className="space-y-8 p-6 max-w-[1600px] mx-auto animate-in fade-in duration-700" dir={dir}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Card className="!p-5 bg-obsidian-card border-border relative overflow-hidden group hover:border-brand/30 transition-all duration-500">
                    <div className="flex items-start justify-between mb-2">
                        <div className="space-y-1">
                            ...
                            <h3 className="text-3xl font-black text-zinc-text tracking-tighter italic tabular-nums">{stats?.activeCampaigns || 0}</h3>
                        </div>
                ...
                <Card className="!p-5 bg-obsidian-card border-border relative overflow-hidden group hover:border-emerald-500/30 transition-all duration-500">
                    <div className="flex items-start justify-between mb-2">
                        <div className="space-y-1">
                            <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest italic font-bold">Total Conversion</span>
                            <h3 className="text-3xl font-black text-emerald-400 tracking-tighter italic tabular-nums">${stats?.totalRevenue?.toLocaleString() || '0'}</h3>
                        </div>
                ...
                <Card className="!p-5 bg-obsidian-card border-border relative overflow-hidden group hover:border-info/30 transition-all duration-500">
                    <div className="flex items-start justify-between mb-2">
                        <div className="space-y-1">
                            <span className="text-[10px] font-black text-info uppercase tracking-widest italic font-bold">Network Reach</span>
                            <h3 className="text-3xl font-black text-info tracking-tighter italic tabular-nums">{stats?.totalParticipants || 0}</h3>
                        </div>
                ...
                <Card className="!p-5 bg-obsidian-card border-border relative overflow-hidden group hover:border-primary/30 transition-all duration-500">
                    <div className="flex items-start justify-between mb-2">
                        <div className="space-y-1">
                            <span className="text-[10px] font-black text-primary uppercase tracking-widest italic font-bold">Success Delta</span>
                            <h3 className="text-3xl font-black text-primary tracking-tighter italic tabular-nums">{stats?.completedCampaigns || 0}</h3>
                        </div>
            ...
            <div className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <h2 className="text-sm font-black text-zinc-muted uppercase tracking-[0.25em] italic flex items-center gap-2">
                        <Layers className="w-4 h-4" /> Operational Campaign Grid
                    </h2>
                    <div className="relative group min-w-[320px]">
                        ...
                        <AmberInput 
                            placeholder="Scan campaign nomenclature..." 
                            className="h-11 bg-obsidian-card border-border pl-11 pr-4 text-xs font-bold italic shadow-inner"
                            value={searchQuery}
                        ...
                        {campaigns.map((campaign) => {
                            ...
                            return (
                                <Card 
                                    ...
                                    onClick={() => navigate(`/group-buying/${campaign.id}`)}
                                >
                                    {/* Visual Coverage */}
                                    <div className="h-56 bg-obsidian-panel/50 relative overflow-hidden">
                                         {campaign.item?.images?.[0] ? (
                                             ...
                                             <div className="w-full h-full flex items-center justify-center opacity-20">
                                                 <Users className="w-16 h-16 text-zinc-muted" />
                                             </div>
                                         )}
                                         {/* State Indicators */}
                                         <div className="absolute top-5 left-5 right-5 flex items-center justify-between">
                                             <StatusBadge 
                                                ...
                                                className="font-black tracking-[0.2em] italic backdrop-blur-xl bg-black/60 px-4 py-1.5 border-none shadow-lg"
                                             />
                                         ...
                                         <div className="absolute bottom-5 left-6 right-6">
                                             ...
                                             <h4 className="text-xl font-black text-white leading-none uppercase italic mt-1 tracking-tighter truncate">{campaign.title}</h4>
                                         </div>
                                    </div>
                                    ...
                                    <div className="p-7 space-y-7">
                                         ...
                                         <div className="space-y-3">
                                              <div className="flex items-center justify-between">
                                                  <span className="text-[10px] font-black text-zinc-muted uppercase tracking-[0.2em] italic">Threshold Synchronization</span>
                                                  <span className="text-[10px] font-black text-brand italic">{Math.round(progress)}% COMPLETE</span>
                                              </div>
                                              ...
                                              <div className="flex items-center justify-between text-[11px] font-black italic tracking-tighter">
                                                  <span className="text-zinc-text">{campaign.currentParticipants} PARTICIPANTS</span>
                                                  <span className="text-zinc-muted uppercase">Target: {campaign.maxParticipants}</span>
                                              </div>
                                         </div>
                                         ...
                                         <div className="grid grid-cols-2 gap-6">
                                             <div className="space-y-1">
                                                 <span className="text-[9px] font-black text-zinc-muted uppercase tracking-[0.2em] italic">Consolidate Price</span>
                                                 <p className="text-2xl font-black text-brand italic tabular-nums leading-none tracking-tighter">${campaign.dealPrice.toLocaleString()}</p>
                                             </div>
                                             <div className="space-y-1 text-right">
                                                 <span className="text-[9px] font-black text-zinc-muted uppercase tracking-[0.2em] italic">Original Premium</span>
                                                 <p className="text-zinc-muted text-lg font-bold italic line-through leading-none tabular-nums tracking-tighter">${campaign.originalPrice.toLocaleString()}</p>
                                             </div>
                                         </div>
                                         ...
                                         <div className="flex items-center gap-3">
                                             <AmberButton 
                                                variant="secondary" 
                                                className="flex-1 h-12 rounded-2xl bg-obsidian-outer hover:bg-brand hover:text-black font-black uppercase text-[10px] tracking-widest italic transition-all active:scale-95 border-border shadow-md"
                                             >
                                                 Analyze Node
                                             </AmberButton>
                         ...
                         <table className="w-full text-left border-collapse" dir={dir}>
                             <thead>
                                 <tr className="border-b border-border bg-obsidian-panel/40">
                                     <th className="px-8 py-5 text-[10px] font-black text-zinc-muted uppercase tracking-widest italic">Identifier Cluster</th>
                                     <th className="px-8 py-5 text-[10px] font-black text-zinc-muted uppercase tracking-widest italic">Node Status</th>
                                     <th className="px-8 py-5 text-[10px] font-black text-zinc-muted uppercase tracking-widest italic">Consolidation Progress</th>
                                     <th className="px-8 py-5 text-[10px] font-black text-zinc-muted uppercase tracking-widest italic text-right">Capital Allocation</th>
                             ...
                             <tbody>
                                 {campaigns.map((campaign) => {
                                     const progress = (campaign.currentParticipants / campaign.maxParticipants) * 100;
                                     return (
                                         <tr key={campaign.id} className="border-b border-border/5 hover:bg-white/[0.02] transition-all cursor-pointer group" onClick={() => navigate(`/group-buying/${campaign.id}`)}>
                                             <td className="px-8 py-6">
                                                 <div className="flex items-center gap-5">
                                                     <div className="w-12 h-12 rounded-xl bg-obsidian-outer border border-border flex items-center justify-center overflow-hidden shrink-0 group-hover:border-brand/40 transition-colors shadow-inner">
                                                         {campaign.item?.images?.[0] ? <img src={campaign.item.images[0]} className="w-full h-full object-cover" /> : <Users className="w-5 h-5 text-zinc-muted" />}
                                                     </div>
                                                     <div>
                                                         <p className="text-sm font-black text-zinc-text uppercase italic tracking-tight">{campaign.title}</p>
                                                         <p className="text-[9px] font-black text-zinc-muted uppercase tracking-widest mt-1 italic">{campaign.category?.name || 'GENERAL'}</p>
                                                     </div>
                                                 </div>
                                             </td>
                                             <td className="px-8 py-6">
                                                 <StatusBadge status={campaign.status.toUpperCase()} variant={getStatusVariant(campaign.status)} size="sm" className="font-black italic px-3 py-1" />
                                             </td>
                                             <td className="px-8 py-6">
                                                 <div className="w-48 space-y-2">
                                                     ...
                                                     <div className="flex justify-between text-[10px] font-black italic">
                                                        <span className="text-zinc-text">{campaign.currentParticipants} NODE REACH</span>
                                                        <span className="text-zinc-muted">{Math.round(progress)}%</span>
                                                     </div>
                                                 </div>
                                             </td>
                                             <td className="px-8 py-6 text-right">
                                                 <p className="text-lg font-black text-brand italic tabular-nums leading-none tracking-tighter">${campaign.dealPrice.toLocaleString()}</p>
                                                 <p className="text-[9px] font-black text-zinc-muted uppercase tracking-widest italic mt-1 line-through">${campaign.originalPrice.toLocaleString()}</p>
                                             </td>
                                             <td className="px-8 py-6 text-right">
                                                 <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                                    <Link to={`/group-buying/${campaign.id}/edit`} onClick={(e) => e.stopPropagation()}>
            ...
            <AmberSlideOver
                ...
                title="Operational Matrix Configuration"
                description="Refine campaign node visibility by status, reach, and Category protocols."
            >
                <div className="space-y-8 py-6">
                    <div className="space-y-3">
                        <label className="text-[10px] font-black text-zinc-muted uppercase tracking-widest italic">Protocol Nomenclature Scan</label>
                        <AmberInput 
                            placeholder="Enter campaign identifier..." 
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            className="h-12 bg-obsidian-outer italic"
                        />
                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="space-y-3">
                            <label className="text-[10px] font-black text-zinc-muted uppercase tracking-widest italic">Node State Strategy</label>
                            <div className="grid grid-cols-2 gap-3">
                                {['all', 'active', 'scheduled', 'completed', 'cancelled'].map((status) => (
                                    <button
                                        ...
                                        className={cn(
                                            "h-11 px-4 rounded-xl text-[10px] font-black uppercase italic tracking-widest transition-all border",
                                            statusFilter === status 
                    ...
                    <div className="pt-10 space-y-4">
                        <AmberButton className="w-full h-14 bg-zinc-text text-black font-black uppercase tracking-[0.3em] italic active:scale-95 transition-all rounded-2xl shadow-xl" onClick={() => setIsFilterOpen(false)}>
                            Reinitialize Scan
                        </AmberButton>
                        <AmberButton 
                            variant="secondary" 
                            className="w-full h-12 font-black uppercase tracking-widest border border-white/5 active:scale-95 transition-all rounded-xl italic"
                            onClick={() => {
                                setStatusFilter('all');
                                setSearchQuery('');
