...
export const Topbar = ({ onOpenSidebar }: { onOpenSidebar: () => void }) => {
  ...
  const handleProjectSwitch = (id: string) => {
    selectProject(id);
    setActiveDropdown(null);
    navigate(paths.dashboard); // Redirect to dashboard of selected project
  };
  ...
  return (
    <header className="sticky top-0 z-40 h-16 bg-obsidian-panel border-b border-border px-6 flex items-center justify-between shadow-lg">
      ...
      {/* Project Switcher */}
      <div className="flex-1 max-w-xs mx-6 hidden md:block relative" ref={dropdownRef}>
        <button 
          onClick={() => toggleDropdown('project')}
          className={cn(
            "w-full h-10 flex items-center justify-between gap-3 px-4 bg-obsidian-outer border border-border rounded-sm hover:border-brand/30 hover:bg-white/[0.02] transition-all group",
        ...
        >
          <div className="flex items-center gap-3 overflow-hidden">
             ...
             <div className="flex flex-col items-start truncate">
                <span className="text-[9px] font-black text-zinc-muted uppercase tracking-widest">Workspace</span>
                <span className="text-xs font-bold text-zinc-text truncate">{activeProject?.name || 'Select Project'}</span>
             </div>
          </div>
        ...
        {activeDropdown === 'project' && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-obsidian-card border border-border rounded-sm shadow-2xl py-1 animate-in fade-in zoom-in-95 duration-100 z-50">
